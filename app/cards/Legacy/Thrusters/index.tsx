...
export function LegacyThrusters() {
	...
	return (
		<div className="grid h-full grid-cols-7 grid-rows-3 items-center gap-4">
			<DamageOverlay systemId={thrusters.id} />
			<div className="col-span-2 row-span-3">
				<p className="mb-2 text-center">Direction</p>
				<div className="flex flex-col gap-4">
					<Joystick
						id="direction"
						className="aspect-square w-full shrink-0 grow-0"
			...
			<div className="col-span-2 row-span-3">
				<p className="mb-2 text-center">Rotation</p>
				...
				<div className="flex flex-col gap-4">
					...
					<LinearJoystick
						...
						gamepadKey="z-thrusters"
					>
						<UntouchableLabel className="right-1">Yaw Starboard</UntouchableLabel>
...
function RotationIndicator({
	...
	return (
		<div>
			<div className="relative aspect-square w-full rounded-full border-2 border-gray-400 bg-gray-500 shadow-[inset_0_0_30px_rgba(0,0,0,0.5)]">
				<div className="absolute top-0 left-1/2 -translate-x-1/2 text-center">0</div>
				<div className="absolute top-1/2 right-0 -translate-y-1/2 text-center">90</div>
				<div className="absolute bottom-0 left-1/2 -translate-x-1/2 text-center">180</div>
				<div className="absolute top-1/2 left-0 -translate-y-1/2 text-center">270</div>
				<div
					className="absolute top-1/2 left-1/2 h-1/3 w-1 origin-bottom rounded-t bg-yellow-300"
					style={{
						transform: `translate(-50%, -100%) rotate(${rotation}deg)`,
					}}
				/>
				<div
					className="absolute top-1/2 left-1/2 h-1/4 w-0.5 origin-bottom -translate-x-1/2 -translate-y-full rounded-t bg-red-500"
					style={{
						transform: `translate(-50%, -100%) rotate(${required}deg)`,
					}}
				/>
			</div>
			<p className="text-center" data-testid={`indicator-${JSON.stringify(children)}`}>
...
function ShipView({ ref }: { ref: Ref<ShipViewRef> }) {
	...
	useImperativeHandle(ref, () => ({ rotation, direction }) => {
		...
		if (direction) {
			...
			if (starboardRef.current && !Array.isArray(starboardRef.current.material)) {
	...
	return (
		...
		>
			<group ref={groupRef}>
				<Arrow ref={portRef} position={[1.5, 0, 0]} rotation={[Math.PI / 2, 0, 0]} />
				<Arrow ref={starboardRef} position={[-1.5, 0, 0]} rotation={[Math.PI / 2, Math.PI, 0]} />
				<Arrow ref={upRef} position={[0, 1.5, 0]} rotation={[0, 0, Math.PI / 2]} />
				<Arrow ref={downRef} position={[0, -1.5, 0]} rotation={[0, 0, -Math.PI / 2]} />
				<Arrow ref={foreRef} position={[0, 0, 1.5]} rotation={[Math.PI / 2, 0, Math.PI / 2]} />
				<Arrow ref={aftRef} position={[0, 0, -1.5]} rotation={[Math.PI / 2, 0, -Math.PI / 2]} />
