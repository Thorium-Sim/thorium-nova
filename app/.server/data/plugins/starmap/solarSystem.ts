...
export const solarSystem = t.router({
	...
	delete: t.procedure
		...
		.send(async ({ ctx, input }) => {
			...
			plugin.aspects.solarSystems.splice(plugin.aspects.solarSystems.indexOf(solarSystem), 1);
	...
	update: t.procedure
		...
		.send(({ ctx, input }) => {
			...
			if (input.habitableZoneInner) solarSystem.habitableZoneInner = input.habitableZoneInner;
			if (input.habitableZoneOuter) solarSystem.habitableZoneOuter = input.habitableZoneOuter;
