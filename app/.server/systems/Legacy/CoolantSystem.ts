...
export class LegacyCoolantSystem extends System {
	...
	update(entity: Entity, elapsed: number) {
		...
		if (coolant.cooling && coolant.coolant > 0 && heat.heat > heat.nominalHeat) {
			const heatAdjustment = heat.maxHeat - heat.nominalHeat;
			const newCoolant = Math.min(
				1,
				Math.max(0, coolant.coolant - coolRate * elapsedRatio * coolant.coolantConsumptionRate),
			);
			...
			const newHeat = Math.max(
				heat.nominalHeat,
				Math.min(heat.maxHeat, heat.heat - heatRemoveRate * heatAdjustment * elapsedRatio),
			);
