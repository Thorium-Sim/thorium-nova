		...
		if (damageComponent && !damageComponent.offline) {
			...
			if (spontaneousFailureChance > 0) {
				...
				if (randomRoll < spontaneousFailureChance) {
					...
					const damageNeededToGoOffline =
						damageComponent.offlineDamage - currentDamage;
					// Add a random amount of damage to possibly damage the system more than needed
					const randomExtraDamage =
						this.ecs.rng.nextAsPercentage() *
						damageComponent.offlineDamage *
						0.1;
					// Take the system offline
					applySystemDamage(
						entity,
						damageNeededToGoOffline + randomExtraDamage,
					);
				}
			}
		}
