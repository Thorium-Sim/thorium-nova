...
export class PhasersSystem extends System {
	...
	update(entity: Entity, elapsed: number) {
		...
		const phaserShip = entity.ecs?.getEntityById(entity.components.isShipSystem?.shipId || -1);
		...
		const phaserDamage = power.currentPower * efficiency * elapsedHours * phasers.yieldMultiplier;
...
export function getTargetIsInPhaserRange(phasers: Entity) {
	...
	const ship = phasers.ecs?.getEntityById(phasers.components.isShipSystem?.shipId || -1);
	if (!ship) return false;
	const target = getCurrentTarget(ship);
	if (!target) return false;

	const { maxRange, arc, maxArc, headingDegree, pitchDegree } = phasers.components.isPhasers;
...
export function getCurrentTarget(ship: Entity) {
	for (const [entityId] of ship.components.shipSystems?.shipSystems || []) {
		...
		if (entity?.components.isTargeting) {
			return ship.ecs?.getEntityById(entity.components.isTargeting?.target || -1);
		}
	}
}
