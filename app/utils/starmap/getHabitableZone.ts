...
export default function getHabitableZone(radius: SolarRadius, temperature: Kelvin) {
	const min = astronomicalUnitToKilometer(0.7 * radius * (temperature / 5777) ** 2);
	const max = astronomicalUnitToKilometer(1.5 * radius * (temperature / 5777) ** 2);
	return { min, max };
}
