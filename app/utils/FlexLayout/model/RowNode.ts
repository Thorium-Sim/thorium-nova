...
import { ROW_NODE } from "@thorium/utils/FlexLayout/model/NODE_TYPES";

export class RowNode extends Node implements IDropTarget {
	static readonly TYPE = ROW_NODE;
	...
	/** @internal */
	_getSplitterBounds(splitterNode: SplitterNode, useMinSize = false) {
		const pBounds = [0, 0];
		const drawChildren = this._getDrawChildren() as (
			| RowNode
			| TabSetNode
		...
		if (this.getOrientation() === Orientation.HORZ) {
			...
			pBounds[1] =
				node2.getRect().getBottom() - splitterNode.getHeight() - minSize2;
		}
		return pBounds;
	}
