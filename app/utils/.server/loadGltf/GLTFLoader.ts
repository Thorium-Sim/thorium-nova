...
class GLTFLightsExtension {
	...
	createNodeAttachment(nodeIndex) {
		...
		return this._loadLight(lightIndex).then((light) =>
			parser._getNodeRef(this.cache, lightIndex, light),
		);
	}
}
...
class GLTFParser {
	...
	constructor(json: { [key: string]: any } = {}, options: { [key: string]: any } = {}) {
		this.json = json;
		...
		return this.getDependency("texture", mapDef.index).then((texture) => {
			...
			if (this.extensions[EXTENSIONS.KHR_TEXTURE_TRANSFORM]) {
				...
				if (transform) {
					...
					texture = this.extensions[EXTENSIONS.KHR_TEXTURE_TRANSFORM].extendTexture(
						texture,
						transform,
					);
	...
	loadMaterial(materialIndex: number): Promise<Material> {
		...
		let materialType;
		const materialParams: Partial<ExcludeFunctionProps<GLTFMeshStandardSGMaterial>> = {};
	...
	loadNode(nodeIndex: number): Promise<Object3D> {
		...
		return (() => {
			...
			const meshPromise = this._invokeOne(
			...
			if (nodeDef.camera !== undefined) {
				pending.push(
					this.getDependency("camera", nodeDef.camera).then((camera) =>
						this._getNodeRef(this.cameraCache, nodeDef.camera, camera),
					),
				);
			}
...
function toTrianglesDrawMode(geometry: BufferGeometry, drawMode: number): BufferGeometry {
