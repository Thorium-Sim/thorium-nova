...
/** Count a literal character in a string, skipping string literals. */
...
function checkDivertsInLine(line, lineNum, rawLine, markers) {
  ...
  let m;
  // biome-ignore lint/suspicious/noAssignInExpressions: All hail the AI gods
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (/^\s*=(?!=)/.test(raw)) {
        ...
        if (inFunctionKnot) {
          markers.push(
            lineMarker(
              SEV.Error,
              "Functions cannot contain stitches.",
              lineNum,
              raw,
            ),
          );
      ...
      if (RE_DECL_NO_NAME.test(stripped)) {
        ...
        markers.push(
          lineMarker(
            SEV.Error,
            `${kw} declaration is missing a variable name.`,
            lineNum,
            raw,
          ),
        );
      ...
      if (dm) {
        ...
        if (kw === "LIST" && rhs !== undefined && rhs.trim() === "") {
          markers.push(
            lineMarker(
              SEV.Error,
              `LIST '${name}' has no values defined.`,
              lineNum,
              raw,
            ),
          );
      ...
      if (/^\s*INCLUDE\b/.test(stripped)) {
        ...
        if (!filename) {
          markers.push(
            lineMarker(
              SEV.Error,
              "INCLUDE statement is missing a filename.",
              lineNum,
              raw,
            ),
          );
      ...
      if (/^\s*->/.test(stripped)) {
        ...
        if (afterArrow.includes("->")) {
          ...
          for (const seg of segments)
            validateTarget(seg, lineNum, raw, markers);
          continue;
        }
      ...
      if (/^\s*[*+]/.test(stripped)) {
        if (inFunctionKnot) {
          markers.push(
            lineMarker(
              SEV.Error,
              "Functions cannot contain choices (* or +).",
              lineNum,
              raw,
            ),
          );
