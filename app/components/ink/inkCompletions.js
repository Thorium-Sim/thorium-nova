...
export function extractSymbols(lines) {
	...
	const stripIBC = (l) => l.replace(/\/\*[^*]*(\*(?!\/)[^*]*)*\*\//g, (m) => " ".repeat(m.length));
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (/^\s*={2,}/.test(line)) {
			const m =
				/^\s*={2,}\s*(function\s+)?([a-zA-Z_][a-zA-Z0-9_]*)(\s*\(([^)]*)\))?\s*(={2,})?/.exec(line);
			if (m) {
				const isFunc = !!m[1];
				const name = m[2];
				const params = (m[4] || "")
		...
		// Label
		const labelM = /^\s*[-*+](?:\s*[-*+])*\s*\(([a-zA-Z_][a-zA-Z0-9_]*)\)/.exec(line);
...
function detectContext(lineText, column) {
	...
	const actionKwMatch = /^\s*((?:Action:|Event:)\w*|Act\w*|Eve\w*|Ac\w?|Ev\w?)$/.exec(before);
	if (actionKwMatch) {
	...
	const divertMatch = /->(?!>)\s*([a-zA-Z_][a-zA-Z0-9_]*\.)?([a-zA-Z_][a-zA-Z0-9_]*)?$/.exec(
		before,
	);
	if (divertMatch) {
	...
	const declRhs = /^\s*(VAR|CONST)\s+[a-zA-Z_][a-zA-Z0-9_]*\s*=\s*([a-zA-Z_][a-zA-Z0-9_]*)?$/.exec(
		before,
	);
	if (declRhs)
		return {
...
function actionNameItems(actions, range) {
	return actions.map((action, idx) => {
		...
		const signature = params.length ? `${name} ${params.map((p) => `${p}: …`).join(" ")}` : name;
		...
		let insertTextRules;
		if (params.length) {
			const paramSnippets = params.map((p, i) => `${p}: \${${i + 1}:${p}}`).join(" ");
	...
	return events.map((event, idx) => {
		...
		return {
			...
			documentation: {
				value: [
					`**Event listener:** \`${name}\``,
					params.length ? `**Match parameters:** ${params.map((p) => `\`${p}\``).join(", ")}` : "",
...
function eventPersistItems(symbols, range) {
	...
	for (const sym of symbols) {
		if (sym.kind !== "knot" && sym.kind !== "stitch" && sym.kind !== "label") continue;
		items.push({
			label: sym.fullName,
			kind: CIK.Reference,
			detail: sym.detail,
...
function eventDivertItems(symbols, range) {
	return symbols
		.filter((s) => s.kind === "knot" || s.kind === "stitch" || s.kind === "label")
		.map((sym) => ({
			label: sym.fullName,
			kind: CIK.Reference,
			detail: sym.detail,
...
function logicItems(symbols, range) {
	...
	for (const sym of symbols) {
		if (!["var", "const", "list", "list_item", "temp"].includes(sym.kind)) continue;
		items.push({
			label: sym.name,
			kind:
				sym.kind === "const"
...
function declarationRhsItems(symbols, range) {
	...
	for (const sym of symbols.filter((s) => ["knot", "function"].includes(s.kind))) {
		result.push({
			label: `-> ${sym.fullName}`,
			kind: CIK.Reference,
			detail: sym.detail,
	...
	return {
		...
		provideCompletionItems(model, position) {
			...
			if (ctx.type === "action_keyword") {
				return {
					suggestions: hashKeywordItems(ctx.matched, position.lineNumber, position.column),
				};
			}
			...
			if (ctx.type === "divert") {
				const knotsAndFunctions = symbols.filter(
					(s) => s.kind === "knot" || s.kind === "function" || s.kind === "label",
				);
				const suggestions = [
			...
			if (ctx.type === "logic") return { suggestions: logicItems(symbols, range) };
			if (ctx.type === "declaration_rhs")
				return { suggestions: declarationRhsItems(symbols, range) };
			if (ctx.type === "identifier")
				return {
					suggestions: [...logicItems(symbols, range), ...lineStartItems(range)],
				};
