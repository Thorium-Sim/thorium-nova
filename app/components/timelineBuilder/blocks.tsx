...
import { getHasMacroSlotBlock, MacroBlock } from "@thorium/components/timelineBuilder/MacroBlock";
...
function MacroTriggerBlocks(props: BlockProps) {
	if (props.type !== "Macro") return null;
	const { pluginId, macroId } = props;
	const [macro] = q.plugin.macro.get.useNetRequest({ pluginId, macroId });
	if (!getHasMacroSlotBlock(macro.blocks)) return null;
	return <TriggerBlocks {...props} />;
}

function TriggerBlocks({ timelineType, executionType, macro, update, ...block }: BlockProps) {
	const getActionPresetValues = useGetActionPresetValue(timelineType || "missions");
	if (!("triggerBlocks" in block)) return null;
