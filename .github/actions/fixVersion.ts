...
try {
	...
	const configPath = path.join(process.cwd(), "../kiosk/src-tauri", "tauri.conf.json");
	...
	const cargoPath = path.join(process.cwd(), "../kiosk/src-tauri", "Cargo.toml");
	await Bun.write(configPath, (await Bun.file(configPath).text()).replace(config.version, version));
	await Bun.write(cargoPath, (await Bun.file(cargoPath).text()).replace(config.version, version));
} catch (error) {
