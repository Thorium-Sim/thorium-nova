...
import {HiSelector, HiSparkles, HiX} from "react-icons/hi";
...
export function QueryComponent({
  ...
  const item = component
    ? parseSchema(components[component]?.removeDefault()).find(
        p => p.key === property
      )
    : null;
...
function PropertyCombobox({
  ...
  const filteredProperties = !component
    ...
    : [
        "isPresent",
        "isNotPresent",
        ...parseSchema(components[component].removeDefault()).map(item =>
