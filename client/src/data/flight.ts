...
export const flight = t.router({
  ...
  start: t.procedure
    ...
    .send(async ({ctx, input: {flightName, ships}}) => {
      inputAuth(ctx);
      ...
      for (const ship of ships) {
        ...
        if (ship.startingPoint) {
          ctx.flight.ecs.entities.forEach(e => {
            try {
              ...
              if (startingEntity.components.satellite.parentId) {
                const parent = ctx.flight?.ecs.getEntityById(
              ...
              const parentSystem = getPlanetSystem(ctx, startingEntity);
              position = {
