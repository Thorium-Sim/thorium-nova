...
All `data.ts` files **must** export a `subscriptions` object. The properties of
this object represent different subscription channel names. These channel names
are used by the `pubsub.publish` function to signal that the data has changed
and trigger a subscription publish. Calling `pubsub.publish` usually happens in
an input definition, but it could happen in an infrequently triggered ECS
System.

Each of the channel name properties on the `subscriptions` object has two
methods: `filter` and `fetch`.

#### `filter`

This function is optional. This function makes sure that subscriptions are only
fired for the clients that need them. For example, two clients on two different
player ships might both be subscribed to the 'phasers' channel, but when one of
the ships charges their phasers, only the client on that ship should get the
subscription update.

When `filter` is defined, it is called for every client subscribed to this
channel. It's called with two arguments: `params`, being whatever values are
passed to `pubsub.publish` in addition to the channel name, and `context` which
is an instance of [DataContext](datacontext) for that particular client.
...
If this function returns `true`, the subscription will be fired. Otherwise, it
will ignore the subscription for that client.
...
When `filter` is omitted, every client subscribed to that channel will recieve
every publish.

#### `fetch`

This function is required. This is the function that collects the data that is
needed for the card to operate. Since it is called both when the card first
loads _and_ when any subscription publishes happen, it needs to be stateless.
That means it does not have access to the argument passed in to
`pubsub.publish`.

`fetch` only has one parameter: an instance of [DataContext](datacontext) for
the client running the card. Whatever it returns is sent to the client and made
available with the [useCardData](#usecarddata) hook. This most likely means
finding specific entities associated with the ship the client is assigned to.
