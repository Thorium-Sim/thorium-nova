...
import type { SoundEffect } from "@server/components/soundEffect";
import { pubsub } from "@server/init/pubsub";
import { t } from "@server/init/t";
import { randomFromList } from "@server/utils/randomFromList";
import { z } from "zod";
import { getSoundPosition } from "@server/utils/getSoundPosition";
...
export const effects = t.router({
	...
	sounds: t.procedure
		.filter((payload: SoundPayload | null, { ctx }) => {
			...
			if (sound.clients?.includes(ctx.id)) return true;
			if (
				sound.stations?.some(
					(s) =>
						s.shipId === ctx.flightClient?.shipId &&
						(!s.stationId || s.stationId === ctx.flightClient?.stationId),
				)
			)
				return true;
