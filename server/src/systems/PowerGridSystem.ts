...
export class PowerGridSystem extends System {
	...
	update(entity: Entity, elapsed: number) {
		...
		for (const node of powerNodes) {
			let nodePower = 0;
			for (const systemId of node.components.isPowerNode?.connectedSystems ||
				[]) {
		...
		reactors.sort((a, b) => {
			...
			const aBatteries = a.components.isReactor?.connectedEntities.filter(
				(id) => batteries.find((b) => b.id === id),
			).length;
			const bBatteries = b.components.isReactor?.connectedEntities.filter(
				(id) => batteries.find((b) => b.id === id),
			).length;
		...
		for (const reactor of reactors) {
			...
			while (totalPower > 0) {
				const powerSplit = totalPower / reactorNodes.length;
			...
			while (totalPower > 0) {
				const powerSplit = totalPower / reactorBatteries.length;
		...
		nodeSuppliedPower.forEach((value, key) => {
			...
			if (value < (node?.components.isPowerNode?.powerRequirement || 0)) {
				...
				connectedBatteries.forEach((battery) => {
					const limit =
						battery.components.isBattery?.dischargeRate ||
			...
			if (distributionMode === "evenly") {
				...
				while (value > 0) {
					const powerSplit = value / connectedSystems.length;
